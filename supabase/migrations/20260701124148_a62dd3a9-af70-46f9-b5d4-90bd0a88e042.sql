...
ALTER TABLE public.campaign_keywords
  ALTER COLUMN conversions TYPE numeric USING conversions::numeric;
ALTER TABLE public.campaign_keywords
  ALTER COLUMN conversions SET DEFAULT 0;
ALTER TABLE public.campaign_keywords
...
CREATE OR REPLACE FUNCTION public.import_campaign_metrics(_platform text, _period_start date, _period_end date, _rows jsonb, _dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  DROP TABLE IF EXISTS _staging_rows;
  ...
  LOOP
    ...
    BEGIN
      v_conv := COALESCE(NULLIF(v_row->>'conversions','')::numeric, 0);
  ...
  LOOP
    ...
    v_cpa  := CASE WHEN v_agg.conversions > 0 THEN v_agg.cost / v_agg.conversions ELSE NULL END;
